...
### for legend ###
ax.scatter(0, 0, color = 'g', label=r'$\mathrm{I_{klod} \ [A] \leq 100} $')
ax.scatter(0, 0, color = 'y', label=r'$\mathrm{100 < I_{klod} \ [A] \leq 200} $')
ax.scatter(0, 0, color = 'r', label=r'$\mathrm{200  < I_{klod} \ [A]} $')
...
for study in study_list:
    ...
    for i, my_i_klod in enumerate(I_klod):
        if np.abs(my_i_klod) <=100:
            colors.append('g')
        elif np.abs(my_i_klod) > 100 and np.abs(my_i_klod) <=200:
            colors.append('y')
        else:
            colors.append('r')
...
plt.tight_layout()
plt.savefig('possible_octuople_settings_klod.png')
