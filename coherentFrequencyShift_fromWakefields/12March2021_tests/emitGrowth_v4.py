...
if __name__ == '__main__':
    ...
    zeffs_dampingRate = np.interp(np.abs(freqs), freqZ, ReZ)*np.sign(freqs)*hs
    zeffs_dampingRate /= np.sum(hs)
    zeff_dampingRate = np.sum(zeffs_dampingRate)

    zeffs_coherentDQ = np.interp(np.abs(freqs), freqZ, ImZ) * hs
    ...
    dampingRate = zeff_dampingRate * cst.e**2 *N / (16.0 * np.pi * cst.m_p * gamma * Qx * frev * sigmaz * 2 * np.pi)

    coherentDQ = zeff_coherentDQ * cst.e ** 2 * N / (16.0 * np.pi * cst.m_p * gamma * Qx * frev * sigmaz * 2 * np.pi)

    print(f'damping rate {dampingRate}')
    print(f'coherent DQ {coherentDQ}')
